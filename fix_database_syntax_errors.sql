-- ===============================================================
-- Fixed Database Script - Syntax Errors Corrected
-- This script fixes the syntax errors in the database functions
-- ===============================================================
...
-- Step 4: Create merchant_products table if missing
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'merchant_products') THEN
...
-- Step 5: Create merchant_checkout_sessions table if missing
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'merchant_checkout_sessions') THEN
...
-- Step 6: Create merchant_payments table if missing
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'merchant_payments') THEN
...
-- Step 7: Drop existing functions with syntax errors
...
-- Step 8: Create corrected upsert_merchant_product function
...
-- Step 9: Create corrected create_merchant_checkout_session function
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'merchant_products') THEN
        ...
        RAISE NOTICE '✅ merchant_products policies and permissions set';
    END IF;
    
    -- merchant_checkout_sessions policies
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'merchant_checkout_sessions') THEN
        ...
        RAISE NOTICE '✅ merchant_checkout_sessions policies and permissions set';
    END IF;
END;
$$;
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '==============================================================';
    RAISE NOTICE 'DATABASE FIX COMPLETED - SYNTAX ERRORS FIXED';
    RAISE NOTICE '==============================================================';
