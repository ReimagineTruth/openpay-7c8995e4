-- Add Google Pay (PayMongo google_pay_card) to QR Pay complete RPC
...
ALTER TABLE public.qr_payment_transactions
  ...
  CHECK (method IN (
    'pi', 'wallet', 'virtual_card', 'qr_ph', 'gcash', 'billease', 'bank', 'google_pay', 'pro'
  ));
...
DECLARE
  ...
  v_existing text;
BEGIN
  IF v_method NOT IN ('qr_ph', 'gcash', 'billease', 'bank', 'google_pay') THEN
