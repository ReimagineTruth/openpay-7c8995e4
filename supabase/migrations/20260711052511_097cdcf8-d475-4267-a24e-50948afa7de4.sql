...
BEGIN
  ...
  -- REQUIRE payment BEFORE creating the item
  IF v_fee_amount > 0 THEN
    ...
    -- Credit collector (avoid self-credit no-op tracking; but still fine if same user)
    IF v_fee_collector IS NOT NULL AND v_fee_collector <> v_uid AND p_payment_method <> 'pi' THEN
      ...
      -- Audit trail so user sees the deduction in Activity
      BEGIN
