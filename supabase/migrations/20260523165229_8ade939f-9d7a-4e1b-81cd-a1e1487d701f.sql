...
-- Fix: pay_merchant_checkout_with_wallet — was crediting merchant AND OpenPay settlement (money created)
...
BEGIN
  ...
  -- Debit buyer, credit merchant ONCE. No settlement transfer (was causing phantom double-credit).
...
-- Fix: pay_merchant_checkout_with_virtual_card (6-arg) — was crediting merchant, debiting merchant, then crediting OpenPay (merchant netted 0; this is wrong too)
CREATE OR REPLACE FUNCTION public.pay_merchant_checkout_with_virtual_card(
  p_session_token text, p_card_number text, p_expiry_month integer,
  p_expiry_year integer, p_cvc text, p_note text DEFAULT ''::text
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Debit buyer, credit merchant ONCE. No settlement transfer.
  UPDATE public.wallets SET balance = v_sender_balance - v_wallet_amount, updated_at = now()
