...
-- This fix ensures POS payments go to merchant balance system, not personal wallet

-- Drop the old POS payment function
DROP FUNCTION IF EXISTS public.process_pos_payment_wallet(TEXT, UUID);

-- Create corrected POS payment function that credits merchant balance system
...
BEGIN
  ...
  -- Determine key_mode from merchant POS settings (default to 'live' if not set)
  SELECT COALESCE(mpas.default_mode, 'live')
  INTO v_key_mode
  FROM public.merchant_pos_api_settings mpas
  WHERE mpas.merchant_user_id = v_pos_payment.merchant_user_id;
  ...
  -- Deduct from payer's personal wallet (this is the actual money movement)
  UPDATE public.wallets
  SET balance = v_payer_balance - v_pos_payment.total_amount,
      updated_at = now()
  WHERE user_id = p_payer_user_id;

  -- Create merchant payment record (this credits the merchant balance system)
  INSERT INTO public.merchant_payments (
    ...
    p_payer_user_id,
    v_net_amount,  -- Only net amount goes to merchant balance
    v_pos_payment.currency,
    v_fee_amount,
    'succeeded',  -- Must be 'succeeded' to count in merchant balance
    v_key_mode,   -- Use detected key_mode
    v_pos_payment.id,
    NULL,         -- Will be set after transaction creation
    now()
  ) RETURNING id INTO v_merchant_payment_id;

  -- Create main transaction record (for audit trail)
  INSERT INTO public.transactions (
    ...
    jsonb_build_object(
      ...
      'merchant_payment_id', v_merchant_payment_id,
      'key_mode', v_key_mode,
      'payment_method', 'wallet'
    )
  ) RETURNING id INTO v_transaction_id;

  -- Update merchant payment with transaction ID
  UPDATE public.merchant_payments
  SET transaction_id = v_transaction_id
  WHERE id = v_merchant_payment_id;
