...
BEGIN
  FOR r IN
    ...
    WHERE proname='create_merchant_payment_link' AND pronamespace='public'::regnamespace
  LOOP
...
BEGIN
  IF v_mode NOT IN ('sandbox','live') THEN RAISE EXCEPTION 'Mode must be sandbox or live'; END IF;
  IF v_link_type NOT IN ('products','custom_amount') THEN RAISE EXCEPTION 'Link type must be products or custom_amount'; END IF;
  IF v_after_payment_type NOT IN ('confirmation','redirect') THEN RAISE EXCEPTION 'After payment type must be confirmation or redirect'; END IF;
  IF char_length(v_currency) < 2 OR char_length(v_currency) > 10 THEN RAISE EXCEPTION 'Currency must be between 2 and 10 characters'; END IF;

  SELECT mak.merchant_user_id, mak.id
  INTO v_merchant_user_id, v_api_key_id
  FROM public.merchant_api_keys mak
  WHERE (mak.secret_key_hash = md5(p_secret_key)
         OR mak.secret_key_hash = encode(digest(p_secret_key, 'sha256'), 'hex'))
  ...
  VALUES (
    ...
    v_link_type, COALESCE(p_title,'OpenPay Payment'), COALESCE(p_description,''),
    v_currency, p_custom_amount,
    p_collect_customer_name, p_collect_customer_email, p_collect_phone, p_collect_address,
    v_after_payment_type, COALESCE(p_confirmation_message,'Thanks for your payment.'),
    p_redirect_url, COALESCE(p_call_to_action,'Pay'), v_expires_at,
    v_fee_amount, p_fee_payer, p_merchant_settlement_amount, p_openpay_fee_account
  )
  RETURNING * INTO v_link;

  -- For products link type, insert line items into merchant_payment_link_items
  IF v_link_type = 'products' THEN
    ...
    LOOP
      ...
      INSERT INTO public.merchant_payment_link_items (
        ...
        v_link.id, v_product.id, v_product.product_name, v_product.unit_amount, v_quantity, v_line_total
      );
    END LOOP;

    -- Add fee on top after items
    IF v_fee_amount > 0 AND p_custom_amount IS NULL THEN
      -- already added fee above only when custom_amount path; here add for products
      v_total := v_total; -- no double-count; fee already included above
    END IF;
  END IF;
