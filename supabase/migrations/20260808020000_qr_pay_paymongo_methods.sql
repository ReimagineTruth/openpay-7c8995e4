-- Allow PayMongo QR PH + GCash on QR Pay transactions, and complete externally paid orders.
...
ALTER TABLE public.qr_payment_transactions
  ...
  CHECK (method IN ('pi', 'wallet', 'virtual_card', 'qr_ph', 'gcash', 'pro'));

-- Complete a QR Pay after PayMongo reports payment.paid / intent succeeded.
-- Called by edge functions with the service role (auth.uid() may be null for guests).
...
DECLARE
  ...
  v_existing text;
BEGIN
  IF v_method NOT IN ('qr_ph', 'gcash') THEN
  ...
  -- Idempotent: same PayMongo intent already completed
  ...
  -- Credit merchant OpenPay balance (same pattern as Pi — funds arrive via PayMongo)
