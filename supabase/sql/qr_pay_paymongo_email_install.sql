-- =============================================================================
-- OpenPay QR Pay — PayMongo methods + Lovable receipt email (paste in SQL Editor)
-- Project: araojncyittkahvvpdrn
-- Safe to re-run.
-- =============================================================================

-- 1) Allow all PayMongo / e-wallet method codes on transactions
...
ALTER TABLE public.qr_payment_transactions
  ...
  CHECK (method IN (
    'pi', 'wallet', 'virtual_card',
    'qr_ph', 'gcash', 'maya', 'grab_pay', 'shopee_pay',
    'billease', 'bank', 'google_pay', 'pro'
  ));

-- 2) Complete PayMongo payment → wallet credit + Lovable outbox notify
...
-- 3) Email receipt without Edge Function (Lovable outbox → cron → send)
...
DECLARE
  ...
  v_dispatched int := 0;
BEGIN
  IF v_ref = '' THEN RAISE EXCEPTION 'transaction_ref required'; END IF;
  ...
  SELECT * INTO v_tx
  FROM public.qr_payment_transactions
  WHERE transaction_ref = v_ref AND status = 'succeeded'
  LIMIT 1;
  ...
  INSERT INTO public.email_notifications_outbox(user_id, to_email, subject, body, status, payload)
  VALUES (
    COALESCE(v_tx.payer_user_id, v_tx.merchant_user_id),
    v_email, v_subject, v_body, 'pending',
    jsonb_build_object(
