# OpenPay Partner API — Lovable Integration Prompt

> **Also in the portal:** https://openpy.space/partner-api → **Copy-paste** tab (one-click copy, Client ID prefilled).
>
> **How to use:** Copy everything below the line into Lovable chat. Replace `YOUR_CLIENT_ID` and `YOUR_API_KEY` with values from https://openpy.space/partner-api (signed in).
...
| What | URL |
|------|-----|
| Site | `https://openpy.space` |
| Partner portal (keys) | `https://openpy.space/partner-api` |
| Auth docs | `https://openpy.space/openpay-auth` |
| API base | `https://araojncyittkahvvpdrn.supabase.co/functions/v1/partner-transfer-api` |
| Auth logo | `https://openpy.space/openpay-auth-logo.png` |
| Consent / authorize | `https://openpy.space/connect` |

### Credentials (fill these in)

```
OPENPAY_CLIENT_ID=YOUR_CLIENT_ID
...
- Register the **exact** redirect URI on https://openpy.space/partner-api → Apps & keys.
...
- `https://openpy.space/oauth2/authorize`
...
```html
<a
  href="https://openpy.space/connect?client_id=YOUR_CLIENT_ID&redirect_uri=https://YOUR_APP_DOMAIN/auth/openpay/callback&scope=profile&state=RANDOM_STATE&response_type=code"
...
>
  <img src="https://openpy.space/openpay-auth-logo.png" width="20" height="20" alt="" />
...
export function OpenPayAuthButton({
  ...
  const url = new URL("https://openpy.space/connect");
  ...
  return (
    ...
    >
      <img
        src="https://openpy.space/openpay-auth-logo.png"
        width={20}
        height={20}
        alt=""
      />
...
`POST https://araojncyittkahvvpdrn.supabase.co/functions/v1/partner-transfer-api/oauth/token`
...
export async function openpayTransfer(opts: {
  ...
  return data; // transfer_id, transaction_id, status, sender_balance, …
}
```
...
```json
{
  ...
  "status": "created",
  "expires_at": "…",
  "checkout_url": "https://openpy.space/paybutton/CHARGE_UUID",
  "success_url": "https://YOUR_APP_DOMAIN/pay/success",
  "cancel_url": "https://YOUR_APP_DOMAIN/pay/cancel"
}
```
...
```html
<a
  href="https://openpy.space/paybutton/CHARGE_UUID"
...
>
  <img src="https://openpy.space/openpay-auth-logo.png" width="16" height="16" alt="" />
  Pay with OpenPay
</a>
```
...
```bash
curl -H "Authorization: Bearer opk_live_YOUR_API_KEY" \
  https://araojncyittkahvvpdrn.supabase.co/functions/v1/partner-transfer-api/charges/CHARGE_UUID
```
...
```bash
curl -X POST "https://…/charges/CHARGE_UUID/cancel" \
  -H "Authorization: Bearer opk_live_YOUR_API_KEY"
```
...
| 400 | Validation / transfer error (`error` string in JSON) |
...
- [ ] Prefer `https://openpy.space` links in UI (not `*.lovable.app`)
...
1. **Sign in with OpenPay** button using the blue `#1652f0` style + `https://openpy.space/openpay-auth-logo.png`
...
6. Short in-app note linking to https://openpy.space/partner-api for key management
